...
class FaceAttendanceSystem:
    ...
    def __init__(self):
        ...
        # FIX: The server has no camera. This line must be disabled for the app to be stable.
        # self.video_capture = cv2.VideoCapture(0) 
        self.video_capture = None # Set it to None to prevent errors
    ...
    def record_attendance(self, student_id, status="Present", date_str=None):
        ...
        try:
            ...
            cursor.execute("INSERT INTO attendance (id, date, time, status) VALUES (?, ?, ?, ?)", (student_id, date_str, time_str, status))
            self.conn.commit()
...
def generate_frames():
    # This function will now always show "Camera Error" on the server
    while True:
        error_frame = np.zeros((480, 640, 3), dtype=np.uint8)
        cv2.putText(error_frame, "Camera Not Available on Server", (110, 240), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
def handle_students():
    ...
    if request.method == 'POST':
        ...
        # Save uploads to the persistent /data volume
